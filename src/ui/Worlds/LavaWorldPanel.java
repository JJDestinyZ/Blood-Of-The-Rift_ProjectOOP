...
import ui.MainFrame;           // To talk to the main window
import ui.Characters.Character; // To see the player
import ui.Characters.*;         // To see specific classes like Warrior/Mage
import ui.Mobs.*;               // To see enemies
import ui.Designs.*;            // To use RiftButton/RiftBar
...
public class LavaWorldPanel extends JPanel {
    ...
    private final String[] introLines = {
            "The air here is toxic, thick with ash and sulfur...",
            "Rivers of molten rock flow like bleeding veins.",
            "The heat is suffocating. Even your armor feels hot to the touch.",
            "The Golem awaits in the heart of the volcano..."
    };
    ...
    private final Color MAGMA_RED = new Color(255, 69, 0);
   ...
   // private final Border MAGMA_BORDER = new LineBorder(MAGMA_RED, 3);
    ...
    private JPanel createRetroLayout() {
        ...
        JLabel title = new JLabel("VOLCANIC CORE", SwingConstants.CENTER);
    ...
    private JPanel createVersusPanel() {
        ...
        vsTextLabel.setForeground(MAGMA_RED);
        ...
        vsMobLabel.setBorder(new LineBorder(MAGMA_RED, 4));
    ...
    private void explore(int dir) {
        ...
        String txt = switch(dir) {
            case 1 -> "You scale the Obsidian Wall. The rock burns your hands.";
            case 2 -> "You cross the Magma Bridge. Heat radiates from below.";
            case 3 -> "You approach the Golem's Furnace. The air is heavy.";
            case 4 -> "You walk through the Ash Fields. Everything is gray.";
            default -> "";
        };
        ...
        if (rng.nextInt(100) < 80) {
            ...
            new Timer().schedule(new TimerTask() {
                @Override public void run() {
                    SwingUtilities.invokeLater(() -> {
                        dialoguePane.setText("The volcano rumbles, but nothing attacks.");
                        new Timer().schedule(new TimerTask() {
    ...
    private void triggerEncounter() {
        if(mobsDefeated >= MOBS_UNTIL_BOSS){
            currentMob = new World4Mob.Golem();
        } else {
            int roll = rng.nextInt(3);
            if(roll == 0) currentMob = new World4Mob.LavaImp();
            else if(roll == 1) currentMob = new World4Mob.MagmaBeast();
            else currentMob = new World4Mob.SkeletonHead();
        }
        startVersusSequence();
    }
    ...
    private void startVersusSequence() {
        ...
        if (currentMob instanceof World4Mob.Golem) {
            vsStatusLabel.setText("The ground shakes violently... The Golem has awakened!");
            vsStatusLabel.setForeground(MAGMA_RED);
        } else {
            vsStatusLabel.setText("A " + currentMob.name + " emerges from the lava!");
            vsStatusLabel.setForeground(Color.WHITE);
        }
        ...
        String mobFileName = "Lava Imp"; // Default
        String nameCheck = currentMob.name.toLowerCase();

        if (nameCheck.contains("imp")) mobFileName = "Lava Imp";
        else if (nameCheck.contains("beast")) mobFileName = "Magma Beast";
        else if (nameCheck.contains("skeleton")) mobFileName = "Skeleton Head";
        else if (nameCheck.contains("golem")) mobFileName = "Golem";

        String mPath = "/images/World4Mob/" + mobFileName + ".png";
        ...
        new java.util.Timer().schedule(new TimerTask() {
            ...
            public void run() {
                SwingUtilities.invokeLater(() -> {
                    ...
                    String enemyAtk;
                    if (currentMob instanceof World4Mob.Golem && rng.nextInt(100) < 35) {
                        enemyAtk = ((World4Mob.Golem) currentMob).specialSkill(player);
                    } else {
                    ...
                    if (player.hp <= 0) {
                        new Timer().schedule(new TimerTask() {
                            @Override public void run() { SwingUtilities.invokeLater(() -> {
                                JOptionPane.showMessageDialog(LavaWorldPanel.this, "DEFEATED", "Game Over", JOptionPane.ERROR_MESSAGE);
    ...
    private void endBattle(boolean win) {
        if (win) {
            battleLogPane.setText("VICTORY! The Golem crumbles.");
            new Timer().schedule(new TimerTask() {
                @Override public void run() {
                    SwingUtilities.invokeLater(() -> {
                        if (currentMob instanceof World4Mob.Golem) {
    ...
    private void playWorld4Outro() {
        ...
        String msg1 = "The Golem falls apart, revealing the source of the Rift's corruption.";
        dialoguePane.setText(msg1);

        new java.util.Timer().schedule(new TimerTask() {
            @Override public void run() { SwingUtilities.invokeLater(() -> {
                String msg2 = "A final portal opens... The End is near.\n" +
                        "Prepare yourself for the Final World.";
                dialoguePane.setText(msg2);
                new java.util.Timer().schedule(new TimerTask() {
                    @Override public void run() { SwingUtilities.invokeLater(() -> {

                        int c = RiftDialog.showConfirmDialog(LavaWorldPanel.this,
                                "Enter the Final World?", "THE END AWAITS");

                        if (c == JOptionPane.YES_OPTION) {
                            mainFrame.enterFinalWorld(player);
